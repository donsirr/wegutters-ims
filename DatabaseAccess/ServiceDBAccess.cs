    ...
    internal class ServiceDBAccess
    {
        ...
        public static ObservableCollection<ServiceCategory> GetServiceCategories()
        {
            ...
            using (var conn = GetConnection())
            {
                ...
                using (var cmd = new SQLiteCommand("SELECT * From ServiceCategories ORDER BY ServiceCategoryID LIMIT -1 OFFSET 1;", conn))
                {
        ...
        public static void EditServiceCategory(string categoryName, ServiceCategory category)
        {
            int id = GetServiceCategoryID(category);

            using (var conn = GetConnection())
            {

                using (var cmd = new SQLiteCommand("UPDATE ServiceCategories SET ServiceCategoryName = @newCategoryName WHERE ServiceCategoryID = @id;", conn))
                {
        ...
        public static void DeleteServiceCategory(ServiceCategory category)
        {
            int id = GetServiceCategoryID(category);
            ...
            using (var conn = GetConnection())
            {
                ...
                using (var cmd = new SQLiteCommand("DELETE FROM ServiceCategories WHERE ServiceCategoryID = @id;", conn))
                {
                    cmd.Parameters.AddWithValue("@id", id);
                    cmd.ExecuteNonQuery();
                }
        ...
        public static ObservableCollection<Service> SearchServices(string search)
        {
            ...
            using (var conn = GetConnection())
            {
                ...
                using (var cmd = new SQLiteCommand("""
                                                    ...
                                                    JOIN ServiceCategories sc ON s.ServiceCategoryID = sc.ServiceCategoryID
                                                    JOIN Customers c ON s.CustomerID = c.CustomerID
                                                    WHERE c.CustomerName LIKE @Search OR sc.ServiceCategoryName LIKE @Search OR c.CustomerAddress LIKE @Search;
                                                    """, conn))
                {
