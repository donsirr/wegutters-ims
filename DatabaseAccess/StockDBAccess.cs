    ...
    internal class StockDBAccess
    {
        ...
        public static ObservableCollection<Category> GetCategories()
        {
            ...
            using (var conn = GetConnection())
            {
                ...
                using (var cmd = new SQLiteCommand("SELECT * From Category ORDER BY CategoryID LIMIT -1 OFFSET 1;", conn))
                {
        ...
        public static ObservableCollection<SKU> GetSKUs()
        {
            ...
            using (var conn = GetConnection())
            {
                ...
                using (var cmd = new SQLiteCommand("SELECT * From SKU ORDER BY SKU_ID LIMIT -1 OFFSET 1;", conn))
                {
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
        ...
        public static ObservableCollection<BaseItem> GetBaseItems()
        {
            ...
            using (var conn = GetConnection())
            {
                ...
                using (var cmd = new SQLiteCommand("""
                                                    ...
                                                    ORDER BY b.ItemID LIMIT -1 OFFSET 1;
                                                    """, conn))
                {
                    using (var reader = cmd.ExecuteReader())
                    {
        ...
        public static void EditBaseItem(BaseItem item, SKU sku, string itemName, Category category, string unit, int quantityPerBundle)
        {
            ...
            using (var conn = GetConnection())
            {
                    ...
                    """, conn))
                {
                    ...
                    cmd.Parameters.AddWithValue("@newSKU", GetSKUID(sku));
